...
(() => {
  ...
  function findSubmitButton() {
    ...
    queryAll('button').forEach((button, index) => {
      ...
      if (editorRect) {
        const dx = Math.abs((rect.left + rect.width / 2) - (editorRect.right - 10));
        const dy = Math.abs((rect.top + rect.height / 2) - (editorRect.top + editorRect.height / 2));
  ...
  function armTrustedInputObserver(editor) {
    ...
    add('keydown', event => {
      ...
      log('keydown', { trusted: event.isTrusted, key: event.key, ctrlKey: event.ctrlKey });
    });
    ...
    add('input', event => {
      ...
      log('input', { trusted: event.isTrusted, valueLength: editorValue(editor).length });
    });
  }
  ...
  async function armSubmit(timeoutMs = 30000) {
    ...
    if (!check.ok) {
      throw new Error('Chưa xác nhận prompt bằng input thật. Click editor → Ctrl+A → Ctrl+V, rồi chạy checkPrompt() lại.');
    }
